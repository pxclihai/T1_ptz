...
#define thread_ptz_control_prio                20
ALIGN(RT_ALIGN_SIZE)
static rt_uint8_t thread_sys_monitor_stack[512];
static rt_uint8_t thread_ptz_control_stack[512];
struct rt_thread thread_sys_monitor;

struct rt_thread thread_ptz_control;
...
int rt_application_init(void)
{
    ...
    rt_thread_startup(&thread_sys_monitor);
		...
		rt_thread_init(&thread_ptz_control,
                   "ptz_control",
                   thread_entry_ptz_contorl,
                   RT_NULL,
                   thread_ptz_control_stack,
                   sizeof(thread_ptz_control_stack),
                   thread_ptz_control_prio, 5);
    rt_thread_startup(&thread_ptz_control);
